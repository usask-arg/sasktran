      ...
      SUBROUTINE TMATRIXRANDOM(REFF,VEFF,RAT,NDISTR,AXMAX,R1,R2,NPNAX,
     &                  B,GAM,NKMAX,EPS,NP,LAM,MRR,MRI,DDELT,NPNA,NDGS,
     &                  CEXTIN,CSCAT,WALB,ASYMM,XMU,
     & 			        F11,F22,F33,F44,F12,F34)

      !DLL_EXPORT TMATRIXRANDOM		!  ****** TRUITT WIENSZ *****  make this available through the DLL interface
...
C     Calling tree:
C       TMATRIX
C           SAREA/SURFCH/SAREAC         points over particle surface
C           <for each size distribution>
C                GAUSS                  set points over size distribution
C                DISTRB                 set up distribution with parameters
C               <for each ? radius in size distribution>
C                   CONSTSP               constants for spher. funcs.
C                   VARY                values of funcs.
C                   TMATR0              zeroth call to calc. T-mat
C                   TMAT                get coeffs alpha, beta for expansion
C                   
C                   
C               HOVENR                  check convergence of results obtainedd
C           MATR                        compute phase matrix from coeffs

      IMPLICIT REAL*8 (A-H,O-Z)
      INCLUDE 'tmd.par.f'
C#include 'tmd.par.f'

      REAL*8  LAM,MRR,MRI,X(NPNG2),W(NPNG2),S(NPNG2),SS(NPNG2),SG,
     *        AN(NPN1),R(NPNG2),DR(NPNG2),WALB,
     *        DDR(NPNG2),DRR(NPNG2),DRI(NPNG2),ANN(NPN1,NPN1),
     *        XMU(NPNA),F11(NPNA),F22(NPNA),F33(NPNA),F44(NPNA),
     *        F12(NPNA),F34(NPNA)
...
C     INITIALIZE THE COMMON-BLOCK ARRAYS TO ZERO
      DO I1=1,NPN2
         DO I2=1,NPN2
            TR1(I1,I2)=0
            TI1(I1,I2)=0
      ...
      DO I1=1,NPN6
         ...
         ENDDO
      ENDDO  
c TRUITT WIENSZ: SAVE ORIGINAL DELTA
      DELTAORIG=DDELT
      
      P=DACOS(-1D0)
C CALCULATE THE MODE WIDTH FOR LOGNORMAL DIST	      
      SG=DEXP(DSQRT(B))
...
* TRUITT WIENSZ: REPLACE STOP WITH RETURN, RESTORE DDELT
!         IF (NK.GT.1000) STOP
         DDELT = DELTAORIG
...
* TRUITT WIENSZ: REPLACE STOP WITH RETURN, RESTORE DDELT
            IF (INM1.GE.NPN1) DDELT = DELTAORIG
...
* TRUITT WIENSZ: REPLACE STOP WITH RETURN, RESTORE DDELT
               IF (NGAUSS.GT.NPNG1) DDELT = DELTAORIG
...
*               IF (NGAUSS.GT.NPNG1) STOP
 ...
 7335 FORMAT('                              NMAX1 =', I3,'  DC2=',D8.2,
     &      '  DC1=',D8.2)
               CALL CONSTSP(NGAUSS,NMAX,MMAX,P,X,W,AN,ANN,S,SS,NP,EPS)
...
* TRUITT WIENSZ: REPLACE STOP WITH RETURN, RESTORE DDELT
               IF (NMA.EQ.NPN1) DDELT = DELTAORIG
...
c              PRINT 7337, NGGG,DSCA,DEXT
 ...
 7550       FORMAT ('NMAX1 = ',I3, ', i.e. greater than NPN4.',
     &              ' Execution terminated')
* TRUITT WIENSZ: REPLACE STOP WITH RETURN, RESTORE DDELT
            IF (NMAX1.GT.NPN4) DDELT = DELTAORIG
...
C     &                     BET1(L),BET2(L)           
  ...
  580    FORMAT(D14.8,I8)
         LMAX=L1MAX-1
         CALL MATR(ALPH1,ALPH2,ALPH3,ALPH4,BET1,BET2,LMAX,NPNA,XMU,
     &             F11,F22,F33,F44,F12,F34)
  600 CONTINUE
C      ITIME=MCLOCK()
...
C**********************************************************************
      ...
      SUBROUTINE CONSTSP (NGAUSS,NMAX,MMAX,P,X,W,AN,ANN,S,SS,NP,EPS)
      IMPLICIT REAL*8 (A-H,O-Z)
      INCLUDE 'tmd.par.f'
C#include 'tmd.par.f'
...
* TRUITT WIENSZ: REPLACE STOP WITH RETURN
...
C        180*(I-1)/(N-1) (DEGREES), WHERE I NUMBERS THE ANGLES
      ...
      SUBROUTINE MATR(A1,A2,A3,A4,B1,B2,LMAX,NPNA,XMU,
     *                F11,F22,F33,F44,F12,F34)
      INCLUDE 'tmd.par.f'
C#include 'tmd.par.f'
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*8 A1(NPL),A2(NPL),A3(NPL),A4(NPL),
     *       B1(NPL),B2(NPL),XMU(NPNA),
     *       F11(NPNA),F22(NPNA),F33(NPNA),F44(NPNA),
     *       F12(NPNA),F34(NPNA)
      N=NPNA
C      DN=1D0/DFLOAT(N-1)
C      DA=DACOS(-1D0)*DN

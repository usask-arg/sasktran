...
#if defined(_MSC_VER)
	#define JPLINT_FUNC JPLINT
#else
	#define JPLINT_FUNC jplint_
#endif

extern  void JPLINT_FUNC (double *tjd, long int *targ, long int *cent, double *posvel, long int *err_flg);
...
*/
   ...
   JPLINT_FUNC (&tjd, &targ, &cent, posvel, &err_flg);
   if (err_flg)
      return (error = 2);

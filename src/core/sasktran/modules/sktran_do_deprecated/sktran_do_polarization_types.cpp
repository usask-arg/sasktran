...
void sktran_do_detail::TripleProductDerivativeHolder<1>::reduce(const LayerInputDerivative<1>& layer_deriv, double& deriv) const {
...
void sktran_do_detail::TripleProductDerivativeHolder<3>::reduce(const LayerInputDerivative<3>& layer_deriv, Eigen::Matrix<double, 3, 3>& deriv) const {
    // TripleProduct derivative with respect to thickness is always 0
...
void sktran_do_detail::TripleProductDerivativeHolder<4>::reduce(const LayerInputDerivative<4>& layer_deriv, Eigen::Matrix<double, 4, 4>& deriv) const {
    // TripleProduct derivative with respect to thickness is always 0
...
template class sktran_do_detail::TripleProductDerivativeHolder<1>;
template class sktran_do_detail::TripleProductDerivativeHolder<3>;
template class sktran_do_detail::TripleProductDerivativeHolder<4>;

void sktran_do_detail::InhomogeneousSourceHolder<1>::reduce(const LayerInputDerivative<1>& layer_deriv, double& deriv) const {
    // TripleProduct derivative with respect to thickness is always 0
...
void sktran_do_detail::InhomogeneousSourceHolder<NSTOKES, CNSTR>::reduce(const LayerInputDerivative<NSTOKES>& layer_deriv, Eigen::Vector<double, NSTOKES>& deriv) const {
    deriv = d_by_ssa * layer_deriv.d_SSA;
    for (int l = 0; l < nstr; ++l) {
...
template class sktran_do_detail::InhomogeneousSourceHolder<1>;
template class sktran_do_detail::InhomogeneousSourceHolder<3>;
template class sktran_do_detail::InhomogeneousSourceHolder<4>;


void sktran_do_detail::Radiance<1>::apply_transmission_factor(const Dual<double>& transmission) {
    deriv = deriv * transmission.value + value * transmission.deriv;
    value = value * transmission.value;
}

template <int NSTOKES, int CNSTR>
void sktran_do_detail::Radiance<NSTOKES, CNSTR>::apply_transmission_factor(const Dual<double>& transmission) {
...
bool sktran_do_detail::Radiance<1>::converged(double I, double epsilon) {
    return abs(this->I() / I) < epsilon;
}

template <int NSTOKES, int CNSTR>
bool sktran_do_detail::Radiance<NSTOKES, CNSTR>::converged(double I, double epsilon) {
    bool converged = true;
...
void sktran_do_detail::Radiance<1>::apply_azimuth_expansion(double angle, int m) {
...
template <>
void sktran_do_detail::Radiance<3>::apply_azimuth_expansion(double angle, int m) {
    double cos_daz = cos(m*angle);
...
template <>
void sktran_do_detail::Radiance<4>::apply_azimuth_expansion(double angle, int m) {
    double cos_daz = cos(m*angle);
    double sin_daz = sin(m*angle);
...
template class sktran_do_detail::Radiance<1>;
template class sktran_do_detail::Radiance<3>;
template class sktran_do_detail::Radiance<4>;

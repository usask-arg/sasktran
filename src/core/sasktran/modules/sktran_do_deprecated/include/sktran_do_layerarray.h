...
namespace sktran_do_detail
{
	...
	{
	public:
		// Constructs an array of optical layers.
		OpticalLayerArray(const PersistentConfiguration<NSTOKES, CNSTR>& config,
						  ...
						  RTSDiagnostics* diag,
						  bool include_direct_bounce_in_reflected=true,
                          int wavelidx=-1,
                          const GeometryLayerArray<NSTOKES, CNSTR>* geometry_layers = nullptr
			);
		...
		void computeReflectedIntensities(AEOrder m, const sktran_do_detail::LineOfSight& los);
        ...
        void copyLegendre(VectorDim1<sktran_do_detail::LegendreCoefficient<NSTOKES>>& container, const Eigen::Matrix<double, Eigen::Dynamic, 6>& legendre);
		...
		inline double opticalDepthAt(double altitude) const {
			if (m_optical_state != nullptr) {
				return m_optical_state->opticaldepth_at_altitude(altitude, m_wavel_index);
			}
			else {
		...
		inline double altitudeAt(double optical_depth) const {
			if (m_optical_state != nullptr) {
				return m_optical_state->altitude_at_opticaldepth(optical_depth, m_wavel_index);
			}
			else {
	...
	private:
		void configurePerturbations(double wavlen, const OpticalState<NSTOKES>* opticalstate, RTSDiagnostics* diag);
	...
	private:
		...
		const OpticalState<NSTOKES>*				m_optical_state;

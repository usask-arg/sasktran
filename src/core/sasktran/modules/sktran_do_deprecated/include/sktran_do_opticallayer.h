...
namespace sktran_do_detail
{
	...
	{
		using ScalarDerivativeResult = sktran_do_detail::LayerFundamentalDerivative<NSTOKES>;
	...
	public:
        ...
        void reset_parameters(double scat_ext, double tot_ext, const VectorDim1<LegendreCoefficient<NSTOKES>>& phasef_expansion,
                              double optical_depth_ceiling,
                              double optical_depth_floor,
                              const InputDerivatives<NSTOKES>& input_derivs);
	...
	protected:
		// Optical properties
		const double								M_SSA;
		const double								M_SCAT_EXT;
		const double								M_TOT_EXT;
		const double								M_OPTICALDEPTH_FLOOR;
		const double								M_OPTICALDEPTH_CEILING;
		const double								M_OPTICAL_THICKNESS;
		const double								M_ALT_CEILING;
		const double								M_ALT_FLOOR;
        std::unique_ptr<VectorDim1<sktran_do_detail::LegendreCoefficient<NSTOKES>>>	m_lephasef;
        ...
        // the above Duals?
        TripleProductDerivativeHolder<NSTOKES>& m_triple_product_holder;

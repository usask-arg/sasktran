...
sktran_do_detail::RTESolver<NSTOKES, CNSTR>::RTESolver(const PersistentConfiguration<NSTOKES, CNSTR>& config, OpticalLayerArray<NSTOKES, CNSTR>& layers)
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::configureCache() {
    ...
    int numDeriv = m_layers.inputDerivatives().numDerivative();
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::solve(AEOrder m)
{
	// Environment setup
...
template<int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::assignHomogenousSplusMinus(AEOrder m, OpticalLayer<NSTOKES, CNSTR>& layer
) {
    ...
    sktran_do_detail::TripleProductDerivativeHolder<NSTOKES>& l_upwelling = m_cache.h_l_upwelling;
    sktran_do_detail::TripleProductDerivativeHolder<NSTOKES>& l_downwelling = m_cache.h_l_downwelling;
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::linearizeHomogeneous(AEOrder m,
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::solveHomogeneous(AEOrder m, OpticalLayer<NSTOKES, CNSTR>& layer)
{
    ...
    if(SKTRAN_DO_USE_EIGEN_EIGENSOLVER || CNSTR != -1) {
        ...
        if(errorcode != 0) {
            if(errorcode < 0) {
                throw InternalRuntimeError("An argument to LAPACKE_dgeev had an illegal argument in sktran_do_detail::RTESolver::SolveHomogeneous");
            } else {
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::assignParticularQ(AEOrder m, const OpticalLayer<NSTOKES, CNSTR> &layer, VectorLayerDual<double>&Qplus,
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::solveParticularGreen(AEOrder m, OpticalLayer<NSTOKES, CNSTR>& layer) {
    ...
    const auto &transmission = layer.dual_beamTransmittance(Location::CEILING, m_layers.inputDerivatives());
    ...
    for(SolutionIndex i = 0; i < N * NSTOKES; ++i) {
        ...
        for(uint j = 0; j < N * NSTOKES; ++j) {
            //double negation = sktran_do_detail::stokes_negation_factor<NSTOKES>(j);
            double negation = 1.0;
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::solveBVP(AEOrder m)
{
	// Setup calculation environment
	la::BVPMatrix<NSTOKES> mat(this->M_NSTR, this->M_NLYR);     // Matrix that defines the system for the boundary value problem
	mat.setZero();
	
	Eigen::VectorXd b(this->M_NSTR * NSTOKES * this->M_NLYR);  // Right hand side of the boundary value problem
	...
	if (errorcode != 0) {
		if (errorcode < 0)
			throw InternalRuntimeError("LAPACKE_dgbsv had an illegal argument in "
				"sktran_do_detail::RTESolver::SolveBVP");
		else
	...
	if (errorcode != 0) {
		if (errorcode < 0)
		{
			std::string error = "LAPACKE_dgbtrs had an illegal argument in sktran_do_detail::RTESolver::SolveBVP when solving derivative, error code: " + std::to_string(errorcode);
			...
			throw InternalRuntimeError("LAPACKE_dgetrs failed to compute particular solution derivative "
				...
				"sktran_do_detail::RTESolver::SolveParticualr");
	}
    for (LayerIndex p = 0; p < this->M_NLYR; ++p) {
        auto& solution = m_layers[p].solution(m);
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::bvpTOACondition(AEOrder m, BoundaryIndex p, BVPMatrix& A, VectorDim1<BVPMatrixDenseBlock<NSTOKES>>& d_A) const
{
	// Eq (29)
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::bvpContinuityCondition(AEOrder m, BoundaryIndex p, BVPMatrix& A, VectorDim1<BVPMatrixDenseBlock<NSTOKES>>& d_A) const
{
	// Eq (31)
	// Setup enviroment
	...
	for (StreamIndex i = 0; i < N*NSTOKES; ++i) {
		for (SolutionIndex j = 0, j_top = 0, j_bot = this->M_NSTR*NSTOKES; j < N*NSTOKES; ++j, ++j_top, ++j_bot) {
			...
			mat(i, j_top) = sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * upper.solution.homog_minus(i, j) * upper.optical.streamTransmittance(Location::INSIDE, m, j);
			mat(i, j_bot) = -sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * lower.solution.homog_minus(i, j);
			...
			for (uint k = 0; k < upper.numDeriv; ++k) {
				...
				d_mat(i, j_top) = sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * (upper.solution.d_homog_minus(i, j, k) * upper.optical.streamTransmittance(Location::INSIDE, m, j) +
			...
			for (uint k = 0; k < lower.numDeriv; ++k) {
				...
				d_mat(i, j_bot) = -sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * lower.solution.d_homog_minus(i, j, k);
			}
		...
		for (SolutionIndex j = 0, j_top = this->M_NSTR / 2 * NSTOKES, j_bot = this->M_NSTR*NSTOKES + this->M_NSTR / 2 * NSTOKES; j < N*NSTOKES; ++j, ++j_top, ++j_bot) {
			...
			mat(i, j_top) = sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * upper.solution.homog_plus(i, j);
			mat(i, j_bot) = -sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * lower.solution.homog_plus(i, j) * lower.optical.streamTransmittance(Location::INSIDE, m, j);
			...
			for (uint k = 0; k < lower.numDeriv; ++k) {
				...
				d_mat(i, j_bot) = -sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * (lower.solution.d_homog_plus(i, j, k) * lower.optical.streamTransmittance(Location::INSIDE, m, j) +
			...
			for (uint k = 0; k < upper.numDeriv; ++k) {
				...
				d_mat(i, j_top) = sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * upper.solution.d_homog_plus(i, j, k);
			}
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::bvpGroundCondition(AEOrder m, BoundaryIndex p, BVPMatrix& A, VectorDim1<BVPMatrixDenseBlock<NSTOKES>>& d_A) const
{
	...
	for(StreamIndex i = 0; i < N*NSTOKES; ++i) {
		for(SolutionIndex j = 0; j < N*NSTOKES; ++j) {
			...
			mat(i, j) = sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * sum_vm * layer.streamTransmittance(Location::INSIDE, m, j);

			sum_vp = v_plus(m, layer, i, j);
			mat(i, j + N*NSTOKES) = sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * sum_vp;

			for (uint k = 0; k < numDeriv; ++k) {
				auto& d_mat = d_A[layerDerivStart + k].upper();

				d_sum = d_v_minus(m, layer, i, j, k, input_deriv[layerDerivStart + k]);
				d_mat(i, j) = sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * (d_sum * layer.streamTransmittance(Location::INSIDE, m, j) + sum_vm * layer.d_streamTransmittance(Location::INSIDE, m, j, k, input_deriv[layerDerivStart + k]));

				d_sum = d_v_plus(m, layer, i, j, k, input_deriv[layerDerivStart + k]);
				d_mat(i, j + N*NSTOKES) = sktran_do_detail::stokes_negation_factor<NSTOKES>(i) * d_sum;
			}
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::bvpCouplingCondition_BC1(AEOrder m, BoundaryIndex p, uint& loc, Eigen::VectorXd& b, VectorDim1<Eigen::VectorXd>& d_b) const
{
	...
	for (StreamIndex i = 0; i < N*NSTOKES; ++i) {
        if(m_use_greens_function) {
            ...
            b[loc] = -solution.particular_plus(i);

            for (uint k = 0; k < numDeriv; ++k) {
                d_b[k][loc] = -solution.dual_particular_plus().deriv(k, i);
            }
        }
		loc++;
	}
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::bvpCouplingCondition_BC2(AEOrder m, BoundaryIndex p, uint& loc, Eigen::VectorXd& b, VectorDim1<Eigen::VectorXd>& d_b) const
{
	// Setup enviroment
	assert(p != 0 && p != this->M_NLYR);
	const uint N = this->M_NSTR / 2;
	...
	for (StreamIndex i = 0; i < N*NSTOKES; ++i) {
        ...
        if(m_use_greens_function) {
            ...
            b[loc + N*NSTOKES] = -1.0*u_p_p * u_bt + l_p_p * l_bt;
            b[loc] = -1.0*u_p_m * u_bt + l_p_m * l_bt;

            // We have cross derivatives
            for (uint j = 0; j < input_deriv.size(); ++j) {
                double d_u_p_p, d_u_p_m, d_u_bt, d_l_p_p, d_l_p_m, d_l_bt;

                d_u_p_p = upper.solution.dual_particular_plus().deriv(j, i);
                d_u_p_m = upper.solution.dual_particular_minus().deriv(j, i);

                d_l_p_p = lower.solution.dual_particular_plus().deriv(j, i);
                d_l_p_m = lower.solution.dual_particular_minus().deriv(j, i);

                // The beam transmittances always potentially exist
                d_u_bt = upper.optical.d_beamTransmittance(Location::FLOOR, input_deriv[j], j);
                d_l_bt = lower.optical.d_beamTransmittance(Location::CEILING, input_deriv[j], j);

                d_b[j][loc + N*NSTOKES] = -1.0*(d_u_p_p * u_bt + u_p_p * d_u_bt) + (d_l_p_p * l_bt + l_p_p * d_l_bt);
                d_b[j][loc] = -1.0*(d_u_p_m*u_bt + u_p_m * d_u_bt) + (d_l_p_m*l_bt + l_p_m * d_l_bt);
            }
        }
...
template <int NSTOKES, int CNSTR>
void sktran_do_detail::RTESolver<NSTOKES, CNSTR>::bvpCouplingCondition_BC3(AEOrder m, BoundaryIndex p, uint& loc, Eigen::VectorXd& b, VectorDim1<Eigen::VectorXd>& d_b) const
{
	// Configure
	assert(p == this->M_NLYR);
...
INSTANTIATE_TEMPLATE(sktran_do_detail::RTESolver);
